...
export default function ToyxonaCRM() {
  ...
  // Kompyuterning real sanasiga moslashuv tizimi
  ...
  // Formadagi har bir maydon uchun alohida valyuta turi
  ...
  // Canli konvertatsiya (Forma jarayonida ko'rinib turishi uchun)
  ...
  // Formadagi jami hisob-kitoblar (Canli)
  ...
  const handleAddCompanyExpense = () => {
    ...
    // Maishiy xarajat kiritilganda kursni shu zahoti qulflaymiz
    const rate = Math.max(1, exchangeRate);
  ...
  // MUHIM: Buyurtmani saqlashda o'sha kundagi kursni butunlay qulflash (Snapshot)
  const handleSaveBooking = () => {
    ...
    const rate = Math.max(1, exchangeRate); // Amaldagi qulflangan kurs
    ...
    const newBooking = {
      ...
      status: 'Tasdiqlangan',
      lockedExchangeRate: rate, // Tarixiy kurs saqlandi
      
      // Barcha qiymatlar ikki tomonlama qulflangan holatda yozildi
  ...
  const renderDashboard = () => {
    // Tarixiy snapshot qiymatlaridan foydalangan holda jami kutilayotgan summani hisoblash
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-6 shadow-lg text-white">
            ...
            <div className="mt-4 pt-4 border-t border-indigo-500/30">
              <p className="text-sm text-indigo-100">Keyingi tadbir: <span className="font-bold text-white">{bookings[0]?.date || 'Hozircha yo\'t'}</span></p>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Telefon / Kompyuter vaqtiga to'liq moslashgan jonli taqvim</p>
            </div>
          </div>
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl">
        <div className="flex items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">To'yxona profili, valyuta kursi va xavfsizlik</p>
          </div>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 space-y-6">
            ...
            <div className="bg-green-50 p-4 rounded-xl border border-green-200">
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full p-3 pr-12 border border-green-300 rounded-xl focus:ring-2 focus:ring-green-500 outline-none font-bold text-green-900 bg-white" 
                />
                <span className="absolute right-4 top-3 text-sm font-bold text-slate-400">UZS</span>
              </div>
              <p className="text-[10px] text-green-600 mt-1">Yangi kiritiladigan buyurtmalar shu kurs bo'yicha hisoblanadi. Eski saqlangan hisobotlar buzilmaydi.</p>
            </div>
          </div>
          ...
          {currentUserRole === 'manager' && (
            <div className="pt-8 border-t border-slate-100 mt-6">
              ...
              <button onClick={handleSavePasswords} className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-md flex items-center gap-2">
              ...
              <div className="pt-8 border-t border-slate-100 mt-8">
                <div className="bg-red-50 p-6 rounded-2xl border border-red-200 flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-red-900">Tizim eski xotirasini butunlay tozalash (Reset)</h4>
                      <p className="text-xs text-red-700 mt-1">Agar sizda hali ham eski test ma'lumotlari qolib ketgan bo'lsa, ushbu tugma brauzer keshini tozalab, ilovani mutlaqo toza holatga keltiradi.</p>
                    </div>
                  </div>
  ...
  const renderFinance = () => {
    // QULFLANGAN Tarixiy snapshot qiymatlar yig'indisi
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">Moliyaviy Hisobotlar (Dual Valyuta)</h2>
              <p className="text-sm text-slate-500">Tarixiy kurslar muhrlangan to'liq xavfsiz buxgalteriya</p>
            </div>
          </div>
        </div>

        {/* Jami hisobot bloklari parallel ravishda ikki valyutada chiqadi */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          ...
          <div className="bg-slate-900 p-6 rounded-2xl shadow-lg text-white">
            <p className="text-xs text-slate-400 font-bold uppercase mb-1">Jami Sof Foyda</p>
            <h3 className="text-lg font-bold text-emerald-400">{formatUZS(netProfitValueUZS)}</h3>
            <h3 className="text-sm font-bold text-white">{formatUSD(netProfitValueUSD)}</h3>
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mt-8">
          <div className="p-6 border-b border-slate-100 bg-slate-50/50">
            <h2 className="text-lg font-bold text-slate-800">Tadbirlar bo'yicha hisobot (Kurs qulflangan nusxalar)</h2>
          </div>
            ...
            bookings.slice().reverse().map(b => {
              ...
              return (
                <div key={b.id} className="transition-colors hover:bg-slate-50/30">
                  <div className="p-6 flex flex-col xl:flex-row xl:items-center justify-between cursor-pointer gap-4" onClick={() => setExpandedFinanceId(isExpanded ? null : b.id)}>
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <span className="text-[10px] text-green-700 bg-green-50 px-2 py-0.5 rounded border border-green-200 font-medium">Kurs: 1$ = {b.lockedExchangeRate} UZS</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-6 flex-wrap text-sm">
                      <div>
                        <p className="text-xs text-slate-400 font-bold mb-0.5">Tushum</p>
                        <p className="font-bold text-green-600">{formatUZS(b.incomeUZS)} / {formatUSD(b.incomeUSD)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400 font-bold mb-0.5">Xarajat</p>
                        <p className="font-bold text-red-500">{formatUZS(b.expensesUZS)} / {formatUSD(b.expensesUSD)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400 font-bold mb-0.5">Foyda</p>
                        <p className={`font-bold ${bNetProfitUZS >= 0 ? 'text-indigo-600' : 'text-red-600'}`}>
                          {formatUZS(bNetProfitUZS)} ({formatUSD(bNetProfitUSD)})
                        </p>
                      </div>
                    ...
                    <div className="px-6 pb-6 pt-2 bg-slate-50/50 border-t border-slate-100">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                        <div className="space-y-2">
                          <h4 className="font-bold text-green-700 border-b border-green-200 pb-1">Kirim detallari</h4>
                          ...
                          <div className="flex justify-between"><span>Qo'shimcha Xizmat:</span><span className="font-medium">{formatUZS(b.additionalServicesUZS)} ({formatUSD(b.additionalServicesUSD)})</span></div>
                          <div className="flex justify-between pt-1 border-t border-dashed border-slate-200 font-bold"><span>Olingan Avans:</span><span className="text-green-600">{formatUZS(b.advanceUZS)} ({formatUSD(b.advanceUSD)})</span></div>
                          <div className="flex justify-between bg-amber-50 p-2 rounded border border-amber-100 mt-1 font-bold"><span>Qoldiq qarz:</span><span className="text-amber-700">{formatUZS(b.incomeUZS - b.advanceUZS)}</span></div>
                        </div>
                        <div className="space-y-2">
                          <h4 className="font-bold text-red-700 border-b border-red-200 pb-1">Chiqim detallari</h4>
                          <div className="flex justify-between"><span>Bino / Oshpaz / Ishchilar / Samovar:</span><span className="font-medium">{formatUZS(b.expElectricityUZS + b.expChefUZS + b.expWorkersUZS + b.expSamovarUZS)}</span></div>
                          {b.otherExpenses && b.otherExpenses.length > 0 && (
                            <div className="pl-3 border-l-2 border-red-200 text-xs space-y-0.5">
                              {b.otherExpenses.map((exp, i) => (
                                <div key={i} className="flex justify-between text-slate-600"><span>- {exp.name}:</span><span>{formatUZS(exp.uzs)} ({formatUSD(exp.usd)})</span></div>
                              ))}
                            </div>
                          )}
        ...
        {/* Maishiy xarajatlar bo'limiga ham Valyuta kiritish imkoni qo'shildi */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mt-8">
              ...
              {currentUserRole === 'manager' ? (
                <div className="lg:col-span-1 bg-slate-50 p-5 rounded-xl border border-slate-200 h-fit space-y-4">
                   ...
                   <div>
                     ...
                     <input type="text" placeholder="Masalan: Bino ta'miri" value={newCompExp.title} onChange={(e) => setNewCompExp({...newCompExp, title: e.target.value})} className="w-full p-2.5 text-sm border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-amber-500" />
                   </div>
                   <div>
                     <label className="block text-xs font-bold text-slate-500 mb-1">Summa va Valyuta</label>
                     <div className="flex border border-slate-200 rounded-lg overflow-hidden bg-white focus-within:ring-2 focus-within:ring-amber-500">
                       ...
                       <select value={newCompExp.currency} onChange={(e) => setNewCompExp({...newCompExp, currency: e.target.value})} className="bg-slate-100 px-2 text-xs font-bold border-l border-slate-200 outline-none">
                ...
                <div className="lg:col-span-1 bg-slate-50 p-5 rounded-xl border border-slate-200 h-fit flex flex-col items-center justify-center text-center py-10 opacity-70">
                   <Shield size={32} className="text-slate-400 mb-3"/><h4 className="text-sm font-bold text-slate-600">Sizda ruxsat yo'q</h4>
                </div>
              )}
                   ...
                   <tbody className="divide-y divide-slate-100">
                      ...
                      companyExpenses.map(exp => (
                        <tr key={exp.id}>
                           ...
                           <td className="p-4 font-bold text-red-500 text-right">{formatUZS(exp.amountUZS)} <br/><span className="text-xs text-slate-400">{formatUSD(exp.amountUSD)}</span></td>
  ...
  const renderNewOrder = () => {
    ...
    // Valyuta selectorini chiroyli tarzda render qiluvchi inline funksiya Component
      ...
      <div>
        ...
        <div className="flex border border-slate-200 rounded-xl overflow-hidden bg-white focus-within:ring-2 focus-within:ring-indigo-500 shadow-sm">
          <input 
            type="text" 
            name={name} 
            value={formatNumber(value).toLocaleString('en-US')} 
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500 pb-10">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          {/* Jonli real-vaqt hisoboti (Kurs o'zgarishi natijalarini to'g'ridan to'g'ri ko'rsatadi) */}
          <div className="xl:col-span-1">
            <div className="bg-slate-900 rounded-3xl shadow-2xl p-8 text-white sticky top-24 space-y-6">
              ...
              <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700 text-[11px] text-slate-400">
                 Tizim joriy kompyuter vaqti (<span className="text-indigo-400 font-bold">{new Date().toLocaleDateString('uz-UZ')}</span>) va joriy valyuta kursi bo'yicha jonli hisoblamoqda.
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col justify-center items-center p-4">
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-3xl w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">Xavfsiz buxgalteriya platformasi</p>
          </div>
            ...
            <div className="relative">
              ...
              {isNotifOpen && (
                <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden">
                  <div className="bg-slate-50 px-4 py-3 border-b border-slate-100 flex justify-between items-center"><h3 className="font-bold text-slate-800">Xabarlar</h3><button onClick={() => requestSystemNotificationPermission()} className="text-[10px] text-indigo-600 font-bold">Ruxsatlar</button></div>
